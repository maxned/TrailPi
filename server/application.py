...
@app.route('/TrailPiServer/api/cleanFiles/<list:filenames>', methods=['GET'])
def clean_files(filenames): 
  '''
    clean up the images and zip file generated by the download route
    Arguments:
  ...
  if os.path.exists('trailpi_images.zip'):
    os.remove('trailpi_images.zip')
  else:
    missing_file = True
  
  if missing_file:
    response = jsonify({'status': 'ERROR, one or more files does not exist'})
    return response, 404
